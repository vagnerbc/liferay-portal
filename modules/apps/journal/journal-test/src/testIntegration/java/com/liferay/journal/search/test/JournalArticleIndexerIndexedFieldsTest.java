...
public class JournalArticleIndexerIndexedFieldsTest
	...
	private Map<String, String> _expectedFieldValues(
		JournalArticle journalArticle) throws Exception {
		...
		indexedFieldsFixture.populateUID(JournalArticle.class.getName(),
			journalArticle.getPrimaryKey(), map);
	...
	private void _populateRoles(
		...
		indexedFieldsFixture.populateRoleIdFields(journalArticle.getCompanyId(),
			JournalArticle.class.getName(), journalArticle.getResourcePrimKey(),
			journalArticle.getGroupId(), null, map);
	}
	...
	private static final String _CONTENT = "test content";

	private static final String _TITLE = "test title";
